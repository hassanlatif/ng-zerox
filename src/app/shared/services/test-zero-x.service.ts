...
export class TestERC721ZeroXService {
    ...
    async createMultiAssetOrder() {

        PrintUtils.printScenario('Create Multi Asset ERC721 Order');
        ...
        // const tokenId = new BigNumber('558');
        const tokenIds = [new BigNumber('10'), new BigNumber('11')];
        // 0x v2 uses hex encoded asset data strings to encode all the information needed to identify an asset
        // const makerAssetData = assetDataUtils.encodeERC721AssetData(dummyERC721TokenContract.address, tokenId);

        const erc721AssetData = [
            assetDataUtils.encodeERC721AssetData(dummyERC721TokenContract.address, tokenIds[0]),
            assetDataUtils.encodeERC721AssetData(dummyERC721TokenContract.address, tokenIds[1])
        ];
        ...
        // const mintTxHash1 = await dummyERC721TokenContract.mint.sendTransactionAsync(maker, tokenIds[0], { from: maker });
        // await printUtils.awaitTransactionMinedSpinnerAsync('Mint ERC721 Token - 1', mintTxHash1);


        // Mint a new ERC721 token for the maker
        // const mintTxHash2 = await dummyERC721TokenContract.mint.sendTransactionAsync(maker, tokenIds[1], { from: maker });
        // await printUtils.awaitTransactionMinedSpinnerAsync('Mint ERC721 Token - 2', mintTxHash2);
        ...
        PrintUtils.printData('Setup', [
            // ['Mint ERC721 - token 1', mintTxHash1],
            // ['Mint ERC721 - token 1', mintTxHash1],
        ...
        await printUtils.fetchAndPrintERC721OwnerAsync(dummyERC721TokenContract.address, tokenIds[0]);
        await printUtils.fetchAndPrintERC721OwnerAsync(dummyERC721TokenContract.address, tokenIds[1]);
        ...
        return { signedOrder, orderHashHex, tokenIds };
    }
    ...
    async fillMultiAssetOrder(signedOrder: SignedOrder, orderHashHex: any, tokenIds: any): Promise<void> {
        PrintUtils.printScenario('Fill Order ERC721');
        // Initialize the ContractWrappers, this provides helper functions around calling
        // 0x contracts as well as ERC20/ERC721 token contracts on the blockchain
        const contractWrappers = new ContractWrappers(providerEngine, { networkId: NETWORK_CONFIGS.networkId });
