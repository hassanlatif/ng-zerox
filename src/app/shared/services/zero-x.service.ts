...
export class ZeroXService {
  ...
  async createNFTOrder() {
    ...
    // 0x v2 uses hex encoded asset data strings to encode all the information needed to identify an asset
    const makerAssetData = assetDataUtils.encodeERC721AssetData(imTokenAddress, tokenId);
    const takerAssetData = assetDataUtils.encodeERC20AssetData(etherTokenAddress);

    // Allow the 0x ERC721 Proxy to move IMT on behalf of makerAccount
    // const makerIMTApprovalTxHash = await contractWrappers.erc721Token.setProxyApprovalForAllAsync(
    //   imTokenAddress,
    //   maker,
    //   true
    // );

    const makerERC721ApprovalTxHash = await contractWrappers.erc721Token.setProxyApprovalAsync(
      imTokenAddress,
      tokenId
    );


    await printUtils.awaitTransactionMinedSpinnerAsync('Maker IMT Approval', makerERC721ApprovalTxHash);
  ...
  //   // up to takerAssetAmount
  //   await contractWrappers.exchange.validateFillOrderThrowIfInvalidAsync(signedOrder, takerAssetAmount, taker);
  //   console.log("post- validateFillOrderThrowIfInvalidAsync");
